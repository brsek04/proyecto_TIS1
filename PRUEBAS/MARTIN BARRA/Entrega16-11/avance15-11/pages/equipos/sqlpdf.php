    ...
    {   
        ...
        if ($this->mostrarFuncionarios) {
            ...
            while ($rowFuncionario = $resultFuncionarios->fetch_assoc()) {
                $this->Cell(0, 5, "{$rowFuncionario['funcionario']}: {$rowFuncionario['cantidadEquipos']} equipos", 0, 1, 'L');
            }
        ...
        if (isset($_POST["modeloCheckbox"])) {
            ...
            $queryModelos = "SELECT modelo, COUNT(id) AS cantidadEquipos
                             ...
                             GROUP BY modelo
                             HAVING cantidadEquipos > 0  -- Excluir modelos con 0 equipos
                             ORDER BY cantidadEquipos DESC";
            $resultModelos = mysqli_query($connection, $queryModelos);
        
            while ($rowModelo = $resultModelos->fetch_assoc()) {
                $this->Cell(0, 5, "{$rowModelo['modelo']}: {$rowModelo['cantidadEquipos']} equipo" . ($rowModelo['cantidadEquipos'] != 1 ? 's' : ''), 0, 1, 'L');
            }
        }
        ...
        if (isset($_POST["tipoCheckbox"])) {
            ...
            while ($rowTipo = $resultTipos->fetch_assoc()) {
                $this->Cell(0, 5, "{$rowTipo['tipo']}: {$rowTipo['cantidadEquipos']} equipos", 0, 1, 'L');
            }
        ...
        if (isset($_POST["marcaCheckbox"])) {
            ...
            while ($rowMarca = $resultMarcas->fetch_assoc()) {
                $this->Cell(0, 5, "{$rowMarca['nombreMarca']}: {$rowMarca['cantidadEquipos']} equipos", 0, 1, 'L');
            }
...
while ($row = $result->fetch_assoc()) {
    ...
    $pdf->Cell(17, 10, $row['marcas'], 1, 0, 'C', 0);
